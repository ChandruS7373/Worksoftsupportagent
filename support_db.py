...
def search_knowledge(query: str = "", top_n: int = 5) -> list:
    """
    Search SF cases using SQL LIKE pre-filter + weighted in-memory scoring.
    ...
    if not query.strip():
        ...
        rows = conn.execute(
            ...
            "WHERE (LENGTH(TRIM(comments)) > 10 OR LENGTH(TRIM(description)) > 10) "
    ...
    if not words:
        conn = get_conn()
        rows = conn.execute(
            "SELECT * FROM sf_knowledge ORDER BY synced_at DESC LIMIT 20"
        ).fetchall()
        conn.close()
        return [dict(r) for r in rows[:top_n]]

    # SQL LIKE pre-filter: keep only rows that contain at least one keyword anywhere
    like_parts  = " OR ".join("all_text LIKE ?" for _ in words)
    like_params = [f"%{w}%" for w in words]
    conn = get_conn()
    rows = conn.execute(
        f"SELECT * FROM sf_knowledge WHERE ({like_parts}) "
        f"AND (LENGTH(TRIM(comments)) > 10 OR LENGTH(TRIM(description)) > 10 OR LENGTH(TRIM(subject)) > 5) "
        f"ORDER BY synced_at DESC LIMIT 300",
        like_params,
    ).fetchall()
    conn.close()
    ...
    def score(d: dict) -> int:
        ...
        for w in words:
            ...
            for token in comments.split() + desc.split():
...
def get_all_case_subjects() -> list:
    """Return sf_case_id + subject + short content snippet for AI case identification."""
    conn = get_conn()
    rows = conn.execute(
        """SELECT sf_case_id, case_number, subject,
               SUBSTR(COALESCE(NULLIF(TRIM(comments),''), NULLIF(TRIM(description),''), ''), 1, 200) AS snippet
           FROM sf_knowledge ORDER BY synced_at DESC"""
    ).fetchall()
    conn.close()
    return [dict(r) for r in rows]
